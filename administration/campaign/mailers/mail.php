...
if(isset($_GET['category'])) {
	...
	if (isset($_GET['category']) && trim($_GET['category']) != '') {
		...
		$mailinglistData = $mailinglistObject->getByCategory($code, 'email');
...
if(count($_POST) > 0 && !isset($_GET['category'])) {
	...
	if(count($errorArray) == 0 && $formValid == true) {		
		...
		for($i = 0; $i < count($_POST['mailinglist_code']); $i++) {
			...
			if($mailinglistData) {
				$mailinglistData['category'] = '_campaign_send';		

				$success = $commObject->sendCampaign($mailinglistData, $campaignData);	
			}
		}
		
		header('Location: /administration/campaign/mailers/comms.php?code='.$campaignData['_campaign_code']);	
		exit;		
...
$smarty->display('administration/campaign/mailers/mail.tpl');
?>