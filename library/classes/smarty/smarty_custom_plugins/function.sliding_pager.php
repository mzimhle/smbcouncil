 ...
 * Amended:  			David Murray <david at clickthinking dot com>
 * HTTP:     				http://www.chengfu.net/ ; http://www.clickthinking.com
 * -------------------------------------------------------------
 */
function smarty_function_sliding_pager($params, &$smarty)
{
    ...
    @param  string  callback            			- callback to which the pagenumber will passed
    ...
    @param  boolean jump_first_last     	- whether to use jump to beginning and end links (default: false)
    */
    ...
    $txt_next  = '>';
    $txt_last  = '>>';
    $txt_skip  = '<div class=page_link_dots>&nbsp;...&nbsp;</div>';
    $css_class = 'page_link';
    $current_css_class = 'page_link_current';
    $link_current = true; 
    $jump_first_last = true;  
    ...
    if ($curpage >= 1) {
		/* redundent with below
        $retval .= ($jump_first_last) ? '<div href="'.$callback.'1'.$url_append.'" '.$css_class.'>'.$txt_first.'</div>'.$separator : '' ;
        */
		$retval .= '<div onclick="'.$callback.'(\''.(($curpage <=1) ? 1 : $curpage-1).'\');" class='.$css_class.'>'.$txt_prev.'</div>'.$separator;
    }
	
    
    if ($links[0] != 1 && $jump_first_last) {
        $retval .= '<div onclick="'.$callback.'(\'1\');" class='.$css_class.'>'.$txt_first.'</div>';
       if ($links[0] == 2) {
            $retval .= $separator;
        } else {
            $retval .= $txt_skip;
    ...
    for($i = 0; $i < sizeof($links); $i++) {
        if ($links[$i] != $curpage or $link_current) {
            $retval .= '<div onclick="'.$callback.'(\''.$links[$i].'\');" class='.( ($links[$i] == $curpage) ? $current_css_class : $css_class).'>'.$links[$i].'</div>';
        } else {
            $retval .= $links[$i];
        }
        if ($i < sizeof($links) - 1) {
    ...
    if ($links[sizeof($links) - 1] != $pagecount && $jump_first_last) {
        if ($links[sizeof($links) - 2] != $pagecount - 1) {
            $retval .= $txt_skip;
        } else {
            $retval .= $separator;
        }
		$retval .= '<div onclick="'.$callback.'(\''.$pagecount.'\');" class='.$css_class.'>'.$txt_last.'</div>';
    }
    
	if ($curpage <= $pagecount) {
		$retval .= $separator.'<div onclick="'.$callback.'(\''.(($curpage >= $pagecount) ? $pagecount : $curpage + 1).'\');" class='.$css_class.'>'.$txt_next.'</div>';
        /* redundent with below
		$retval .= ($jump_first_last) ? $separator.'<div href="'.$callback.$pagecount.$url_append.'" '.$css_class.'>'.$txt_last.'</div>' : '' ;
		*/
    }
    return $retval;
}
